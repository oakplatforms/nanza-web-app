...
export function ListingOrBidDetail() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const { listing, isLoadingListing, errorListing } = useFetchListing(id || '')
  const { bid, isLoadingBid, errorBid } = useFetchBid(id || '')
  const { profile, isLoadingProfile } = useFetchProfile(id || '')

  // Determine which one we have
  const isListing = !!listing && !isLoadingListing && !errorListing
  const isBid = !!bid && !isLoadingBid && !errorBid
  const isProfile = !!profile && !isLoadingProfile
  const isLoading = (isLoadingListing && !errorListing) || (isLoadingBid && !errorBid) || isLoadingProfile

  // If it's actually a profile, redirect to username route
  useEffect(() => {
    if (isProfile && id) {
      navigate(`/${id}`, { replace: true })
    }
  }, [isProfile, id, navigate])
  ...
  // Use listing if available, otherwise bid
