...
export function ListingDetail() {
  // Support both :listingId and :id params for flexibility
  const { listingId, id } = useParams<{ listingId?: string; id?: string }>()
  const actualListingId = listingId || id || ''
  const navigate = useNavigate()
  const { listing, isLoadingListing, errorListing } = useFetchListing(actualListingId)
  ...
  if (!entity) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-lg text-red-600">Listing does not have an entity associated.</div>
      </div>
    )
  }
